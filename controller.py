...
class DelayController():
	def __init__(self, _id, max_delay):
		...
		self.q_len_limit = 2
		self.q_len = 0
		self.q_len_max = self.q_len_limit
		self.avg_delay = 0
		self.a = 0.9
	...
	def update(self, t): # t: turnaround_time
		...
		if self.avg_delay > self.max_delay:
			...
			if self.q_len_limit < 2*self.q_len_max:
				self.q_len_limit += 1/self.q_len_limit
		log(DEBUG, "id= {}".format(self._id), avg_delay=self.avg_delay, q_len_limit=self.q_len_limit, q_len=self.q_len)
